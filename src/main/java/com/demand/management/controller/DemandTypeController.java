...
public class DemandTypeController {
    ...
    public DataResponse<RelationDemandType> create(
        ...
        if (res != null) return new DataResponse<RelationDemandType>("创建需求类型成功", true, res);
        else return new DataResponse<RelationDemandType>("创建需求类型失败", false);
    }
    ...
    public PageDataResponse find(
        ...
        return new PageDataResponse("获取需求类型列表成功", true, list.getRecords(), list.getTotal());
    }
    ...
    public DataResponse<RelationDemandTypeWithApprover> getById(@PathVariable String id) {
        ...
        if (role != null) return new DataResponse<RelationDemandTypeWithApprover>("获取id为" +id + "的需求类型成功", true, role);
        else return new DataResponse<RelationDemandTypeWithApprover>("id为" +id + "的需求类型不存在", false);
    }

    @Transactional
    @PutMapping("/{id}")
    public BaseResponse update(@PathVariable String id, @RequestBody DemandTypeBodyReq body) {
        if (!this.service.updateById(id, body)) return new BaseResponse("更新id为" +id + "的需求类型失败", false);
        RelationDemandType role = this.service.findById(id);
        if (role != null) return new BaseResponse("更新id为" +id + "的需求类型成功", true);
        else return new BaseResponse("id为" +id + "的需求类型不存在", false);
    }
}
