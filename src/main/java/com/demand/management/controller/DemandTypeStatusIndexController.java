...
public class DemandTypeStatusIndexController {
    ...
    public DataResponse<List<RelationDemandTypeStatusIndex>> getByDemandTypeId(@PathVariable String typeId) {
        ...
        if (list != null) return new DataResponse<List<RelationDemandTypeStatusIndex>>("获取demandTypeId为" +typeId + "的需求类型顺序成功", true, list);
        else return new DataResponse<List<RelationDemandTypeStatusIndex>>("demandTypeId为" + typeId + "的需求类型顺序不存在", false);
    }
    ...
    public DataResponse<List<RelationDemandTypeStatusIndex>> create(@PathVariable String typeId, @RequestBody List<DemandTypeStatusIndexReqItem> body) {
        ...
        if (list != null) return new DataResponse<List<RelationDemandTypeStatusIndex>>("创建demandTypeId为" +typeId + "的需求类型顺序成功", true, list);
        else return new DataResponse<List<RelationDemandTypeStatusIndex>>("创建demandTypeId为" + typeId + "的需求类型顺序不失败", false);
    }
}
