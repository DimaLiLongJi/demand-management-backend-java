...
public class AuthController {
    ...
    public DataResponse<RelationUser> auth(HttpServletRequest request) {
        ...
        return ManagementResponseUtil.<RelationUser>buildDataResponse("验证成功", true, user);
    }
    ...
    public BaseResponse logout(HttpServletRequest request, HttpServletResponse response) {
        ...
        if (token != null && !token.getValue().isEmpty()) {
            ...
            return ManagementResponseUtil.buildBaseResponse("退出登录成功", true);
        } else {
            return ManagementResponseUtil.buildBaseResponse("用户并未登录", true);
        }
    }
    ...
    public BaseResponse login(@RequestBody LoginResBody info, HttpServletResponse response) throws Exception {
        ...
        // 设置密码
        if (info.getPassword() == null) return ManagementResponseUtil.buildBaseResponse("密码不正确，请再次输入", false);
        ...
        BaseResponse res = ManagementResponseUtil.buildBaseResponse(loginRes.getMessage(), loginRes.isSuccess());
        // 登录成功
